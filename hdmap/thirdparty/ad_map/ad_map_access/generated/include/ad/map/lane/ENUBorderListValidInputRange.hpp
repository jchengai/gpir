 ...
 *
 * \returns \c true if ENUBorderList is considered to be within the specified
 * input range
 *
 ...
 */
inline bool withinValidInputRange(::ad::map::lane::ENUBorderList const &input,
  ...
  if (inValidInputRange) {
    for (auto const &member : input) {
      ...
      if (!memberInValidInputRange && logErrors) {
        spdlog::error(
            "withinValidInputRange(::ad::map::lane::ENUBorderList)>> {}, "
            "invalid member {}",
            input,
