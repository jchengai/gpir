...
void opendrive::parser::ProfilesParser::ParseElevation(
  ...
  for (pugi::xml_node laneSection = xmlNode.child("elevation"); laneSection;
    ...
    elevationProfile.start_position =
    ...
    elevationProfile.vertical_curvature =
        std::stod(laneSection.attribute("c").value());
    elevationProfile.curvature_change =
...
void opendrive::parser::ProfilesParser::ParseLateral(
    const pugi::xml_node &xmlNode,
  ...
  for (pugi::xml_node laneSection = xmlNode.child("superelevation");
    ...
    lateralProfile.vertical_curvature =
        std::stod(laneSection.attribute("c").value());
    lateralProfile.curvature_change =
        std::stod(laneSection.attribute("d").value());
...
void opendrive::parser::ProfilesParser::Parse(
    const pugi::xml_node &xmlNode, opendrive::RoadProfiles &out_road_profiles) {
  opendrive::parser::ProfilesParser profilesParser;

  profilesParser.ParseElevation(xmlNode.child("elevationProfile"),
                                out_road_profiles.elevation_profile);
  profilesParser.ParseLateral(xmlNode.child("lateralProfile"),
                              out_road_profiles.lateral_profile);
}
